...
describe("Admin Config Options", () => {
  ...
  it("Can view admin page", () => {
    ...
    fields.forEach(field => {
      cy.get(`input[name="${field.fieldName}"]`).should('have.value', `C:\\fakepath\\${field.fieldName}.png`);
    });
    ...
    cy.get('#field-ckan\\.welcome_page_description').type(textFields.description);
    ...
    socialMediaLinks.forEach((link) => {
      cy.get(`#${link}`).drag("#column-social-selected", { force: true, target: {
        position: "bottom"
      } });
    ...
    cy.get("#header-wrapper").should("have.css", "background-image").and("include", "headerimageupload.png");
    cy.get("#header-text").should("have.css", "color").and("eq", 'rgb(255, 255, 245)');
    cy.get("#header-title").should("have.text", textFields.title);
    cy.get(".long-desc").should("include.text", textFields.description);
    cy.get('.custom-footer > :nth-child(1) > img').should('have.attr', 'src').and('include', 'footerlogoimageupload.png');
    cy.get('.custom-footer > :nth-child(1) > div > p').should('have.text', textFields.footer_logo_text);
    cy.get('.custom-footer > :nth-child(2) > img').should('have.attr', 'src').and('include', 'footerlogo2imageupload.png');
    cy.get('.custom-footer > :nth-child(2) > div > p').should('have.text', textFields.footer_logo2_text);
